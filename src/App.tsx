...
// ⚠️ IMPORTANTE: PEGA AQUÍ TU NUEVA API KEY COPIADA DE LA CONSOLA DE FIREBASE
              ...
              {type === 'admin' ? (
                <>
                  ...
                  <section>
                    ...
                    <ul className="list-disc pl-5 space-y-4 text-sm"><li><strong>1. Uploading Questions:</strong><div className="mt-2 space-y-3"><div className="bg-slate-950 p-3 rounded border border-blue-900/50"><span className="text-blue-300 font-bold block mb-1">Truth & Dare Files (Use separate buttons):</span><span className="text-slate-400">Headers must be:</span> <code className="text-green-400">text, level, gender</code><br/><span className="text-xs text-slate-500">Gender = <strong>M</strong> for Male, <strong>F</strong> for Female, <strong>B</strong> for Both.</span></div><div className="bg-slate-950 p-3 rounded border border-green-900/50"><span className="text-green-300 font-bold block mb-1">Match Files:</span><span className="text-slate-400">Headers must be:</span> <code className="text-green-400">male, female, level</code></div></div></li><li><strong>Singles vs Couples:</strong> The system <strong>understands</strong> if players are Single or Couples. If Couples join, the game <strong>will not start</strong> until both partners (matching ID) are present.</li><li><strong>Bot System:</strong> If total players are odd, "Brad Pitt" (or "Scarlett Johansson") joins to ensure everyone has a partner in Match rounds.</li></ul>
                  </section>
                </>
              ) : (
                <>
...
export default function TruthAndDareApp() {
  ...
  // FETCH MISSING CARD
  useEffect(() => {
    // FIX APLICADO: Eliminado "&& isAdmin" para que todos los jugadores puedan descargar la carta
  ...
  const handleReturnToSetup = async () => {
      if(confirm('Start a new game (Return to Setup)?')) {
          const batch = writeBatch(db);
          batch.update(doc(db, 'artifacts', appId, 'public', 'data', 'gameState', 'main'), { mode: 'admin_setup', currentTurnIndex: 0, answers: {}, votes: {}, isEnding: false });
          await batch.commit();
      }
  };
  ...
  // --- GAME ENDED SCREEN (TOP PRIORITY) ---
  if (gameState?.mode === 'ended') {
      return (
        <div className="min-h-screen text-white p-6 flex flex-col items-center justify-center bg-slate-900">
            ...
            {isAdmin && (
                <button onClick={handleReturnToSetup} className="bg-blue-600 px-6 py-3 rounded-lg font-bold shadow-lg hover:bg-blue-500 transition-transform active:scale-95 w-full max-w-sm">
                    New Game (Setup)
                </button>
            )}
        </div>
  ...
  if (isAdmin) {
    if (!gameState || !gameState.mode || gameState.mode === 'lobby' || !['admin_setup', 'question', 'dare', 'yn', 'ended'].includes(gameState.mode)) {
        ...
        return (
            <div className="min-h-screen p-6 flex flex-col items-center justify-center text-white bg-slate-900 relative">
              ...
              {!isSettingCode ? (
                  <button onClick={() => setIsSettingCode(true)} className="w-full max-w-sm bg-blue-900/50 border border-blue-600/50 p-2 rounded-lg font-bold mb-4 text-blue-200 hover:bg-blue-800 transition flex items-center justify-center gap-2 text-sm"><Send size={14}/> Set Game Code</button>
              ) : (
                  <div className="w-full max-w-sm flex gap-2 mb-4 animate-in fade-in slide-in-from-top-2"><input type="text" placeholder="Enter Code..." className="bg-slate-800 border border-slate-600 rounded p-2 text-white flex-1 outline-none focus:border-blue-500" value={code} onChange={e=>setCode(e.target.value)} autoFocus /><button onClick={setGameCode} className="bg-green-600 px-3 rounded font-bold hover:bg-green-500">Save</button><button onClick={() => setIsSettingCode(false)} className="bg-red-600 px-3 rounded hover:bg-red-500">X</button></div>
              )}
              ...
              {!couplesValid && <div className="bg-red-900/80 p-3 rounded text-center text-sm mb-4 border border-red-500 max-w-sm w-full animate-pulse"><strong className="block text-red-200 mb-1"><AlertTriangle className="inline mr-1" size={14}/> Incomplete Couples!</strong>Wait for partners for IDs: {incompleteIds.join(', ')}</div>}
              {total > 0 ? (
    ...
    // FIX APLICADO: Definimos cardStyle dentro del scope del admin
    const cardStyle = getLevelStyle(finalCard?.level);

    // FIX: Show syncing state if card is missing (Fixes Blue Screen)
    if (!finalCard && gameState?.currentChallengeId) {
        return <div className="min-h-screen flex flex-col items-center justify-center p-6 text-white bg-slate-900"><div className="text-xl animate-pulse">Syncing card data...</div></div>;
    }
      ...
      <div className="min-h-screen text-white flex flex-col p-6 bg-slate-900 relative">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className={`w-full max-w-md p-8 rounded-3xl border-4 text-center mb-8 transition-all duration-500 ${cardStyle} flex flex-col items-center justify-center min-h-[200px] relative`}>
              <div className="absolute top-4 left-4 text-[10px] font-bold opacity-60 uppercase tracking-widest text-white/50 bg-black/20 px-2 py-1 rounded">
