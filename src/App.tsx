          ...
          <div className="space-y-8 text-slate-300">
            {type === 'admin' && (
              <>
                ...
                <section>
                  <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2"><Zap className="text-yellow-400"/> Game Flow & Control</h3>
                  <div className="space-y-3 text-sm bg-slate-900 p-4 rounded-lg border border-slate-700">
                      <p><strong className="text-white">Flexibility:</strong> You are the boss! You can switch between <strong>Auto</strong> and <strong>Manual</strong> modes at any time during the game.</p>
                      <ul className="list-disc pl-5 space-y-1 text-slate-300">
                          <li><strong>Changing Levels:</strong> You can adjust the <strong>Risk Level</strong> (1-4) at any moment, in both Auto and Manual modes, to control the intensity.</li>
                          <li><strong>Changing Game Type:</strong> In <strong>Manual Mode</strong>, you can override the sequence and select specific game types (Truth, Dare, Match) for the next turn.</li>
                          <li><strong>Force Next Turn:</strong> In Manual Mode, you can use the "Next (Force)" button to skip waiting and advance immediately.</li>
                      </ul>
                  </div>
                </section>
                  ...
                  <ul className="list-disc pl-5 space-y-4 text-sm">
                    <li>
                        ...
                        <div className="mt-2 space-y-3">
                            <div className="bg-slate-950 p-3 rounded border border-blue-900/50"><span className="text-blue-300 font-bold block mb-1">Game Type Truth & Dare:</span><span className="text-slate-400">Use CSV files with headers:</span> <code className="text-green-400">text, level, gender</code><br/><span className="text-xs text-slate-500">Gender = M, F, B. Example: "What is your secret?", 1, B</span></div>
                            <div className="bg-slate-950 p-3 rounded border border-green-900/50"><span className="text-green-300 font-bold block mb-1">Game Type Match:</span><span className="text-slate-400">Use CSV files with headers:</span> <code className="text-green-400">male, female, level</code><br/><span className="text-xs text-slate-500">Example: "I snore", "I snore", 1</span></div>
                        </div>
                    </li>
                    ...
                    <li><strong>Reset All:</strong> <span className="text-red-400 font-bold">Note:</span> The game starts from zero. Use only for a new party.</li>
                  </ul>
                </section>
              </>
            )}
...
export default function TruthAndDareApp() {
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'T' | 'D' | 'M') => {
      ...
      lines.slice(1).forEach(line => {
          ...
          if (type === 'M') {
              ...
              if (male && female && male !== 'male') { // Avoid header row
  ...
  const handleExportCSV = (isTemplate: boolean) => {
      const isTD = managerTab !== 'match';
      const headers = isTD ? "text,level,gender" : "male,female,level"; 
      let csvContent = "data:text/csv;charset=utf-8," + headers + "\n";
      if (!isTemplate) {
          const data = isTD ? challenges.filter(c => c.type === (managerTab === 'truth' ? 'T' : 'D')) : pairChallenges;
          data.forEach(row => {
              if (isTD) {
      ...
      <div className="min-h-screen text-white flex flex-col p-6 bg-slate-900 relative">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className={`w-full max-w-md p-8 rounded-2xl border-2 text-center mb-8 border-indigo-500 bg-indigo-900/20`}>
              <div className="text-2xl font-black text-white/50 uppercase tracking-widest mb-4">
                  {gameState.mode === 'yn' ? (
                      <span>MATCH / MISMATCH {players.find(p=>p.uid===user?.uid)?.relationshipStatus==='couple' ? `- COUPLE #${players.find(p=>p.uid===user?.uid)?.coupleNumber}` : ''}</span>
