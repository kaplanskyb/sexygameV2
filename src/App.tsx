...
import { 
    ...
    HeartHandshake, AlertTriangle, X,
    BookOpen, Zap, CheckCircle, Upload, Download,
    LogOut, MessageCircle, AlertCircle
} from 'lucide-react';
                            ...
                            <>
                                <section>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {/* TRUTH */}
                                        <div className={`cursor-pointer border rounded-xl p-4 transition-all ${expandedSection === 'truth' ? 'bg-blue-900/40 border-blue-400 shadow-[0_0_20px_rgba(59,130,246,0.3)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`} onClick={() => toggleSection('truth')}>
                                            ...
                                            {expandedSection === 'truth' && <div className="mt-4 text-sm text-white border-t border-blue-500/30 pt-2 animate-in fade-in"><p><strong>Voting:</strong> The group votes "Good Answer" or "Boring".</p></div>}
                                        </div>
                                        {/* DARE */}
                                        <div className={`cursor-pointer border rounded-xl p-4 transition-all ${expandedSection === 'dare' ? 'bg-pink-900/40 border-pink-400 shadow-[0_0_20px_rgba(236,72,153,0.3)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`} onClick={() => toggleSection('dare')}>
                                            ...
                                            {expandedSection === 'dare' && <div className="mt-4 text-sm text-white border-t border-pink-500/30 pt-2 animate-in fade-in"><p><strong>Voting:</strong> The group votes "Completed" or "Failed".</p></div>}
                                        </div>
                                        {/* MATCH */}
                                        <div className={`cursor-pointer border rounded-xl p-4 transition-all ${expandedSection === 'match' ? 'bg-emerald-900/40 border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}`} onClick={() => toggleSection('match')}>
                                            ...
                                            {expandedSection === 'match' && <div className="mt-4 text-sm text-white border-t border-emerald-500/30 pt-2 animate-in fade-in"><p><strong>Goal:</strong> Both answer YES or NO. Match = Points.</p></div>}
                                        </div>
                                    </div>
                                </section>
                                <section>
                                    <h3 className="text-xl font-bold text-white mb-3 flex items-center gap-2"><Zap className="text-yellow-400" /> Controls</h3>
                                    <ul className="list-disc pl-5 space-y-2 text-sm text-slate-300">
                                        <li><strong>Manual Mode:</strong> You pick risk/type every turn.</li>
                                        <li><strong>Auto Mode:</strong> Set a loop (e.g., 2 Truths, 1 Match) and just click Next.</li>
                                        <li><strong>Uploading:</strong> Use CSV files with headers: <code>text, level, gender</code> (Truth/Dare) or <code>male, female, level</code> (Match).</li>
                                    </ul>
                                </section>
                            </>
                        )}
                            ...
                            <>
                                <section>
                                    ...
                                    <ul className="list-disc pl-5 space-y-2 text-slate-300">
                                        <li><strong>Couples:</strong> Use the SAME 4-digit code to link together.</li>
                                        <li><strong>Singles:</strong> Enter any 4 digits (like your phone pin).</li>
                                        <li><strong>Game Code:</strong> Ask the Admin/Host for the party code.</li>
                                    </ul>
                                </section>
                                <section>
                                    <h3 className="text-xl font-bold text-white mb-3">🎮 Gameplay</h3>
                                    <p>Follow the instructions on your screen. If it's a <strong>Match Round</strong>, you'll answer secretly. If it's <strong>Truth/Dare</strong>, you perform for the group!</p>
                                </section>
                            </>
                        )}
                    </div>
                    <div className="mt-8 text-center border-t border-white/10 pt-6">
                        <button onClick={onClose} className="bg-gradient-to-r from-cyan-600 to-blue-600 px-8 py-3 rounded-xl font-bold text-white shadow-lg">Got it</button>
                    </div>
                </div>
            </div>
        </div>
...
// --- COMPONENTE PAREJAS (CouplePairing) ---
const CouplePairing = ({ gender, onCodeObtained, value, onBack, onAutoJoin, db, currentUserUid, appId }: any) => {
    ...
    // 1. LÓGICA MUJER: Escuchar la BD para ver si alguien se une
    useEffect(() => {
        if (isFemale && db && localCode) {
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                // Buscamos si hay ALGUIEN MÁS (que no sea yo) con mi mismo código
                const partner = snapshot.docs.find(d => d.data().uid !== currentUserUid);
                if (partner) {
                    setIsLinked(true);
                    onCodeObtained(localCode);
                    // Auto-entrar después de 2 segundos
    ...
    const handleManSubmit = () => {
        ...
        setIsLinked(true);
        // Auto-entrar después de 1.5 segundos
        setTimeout(() => { onAutoJoin(inputCode); }, 1500);
    };
            ...
            <div className="bg-slate-800 border border-white/10 p-8 rounded-3xl w-full max-w-sm shadow-2xl flex flex-col items-center relative">
                ...
                {isFemale ? (
                    ...
                    <>
                        ...
                        <input type="number" inputMode="numeric" pattern="[0-9]*" maxLength={4} placeholder="0000" className="w-full bg-slate-900 border-2 border-slate-700 focus:border-purple-500 text-white font-mono font-black text-5xl text-center py-4 rounded-xl outline-none transition-all mb-8 placeholder:text-slate-700 drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]" value={inputCode} onChange={(e) => setInputCode(e.target.value.slice(0, 4))} />
                        
                        <button onClick={handleManSubmit} disabled={inputCode.length < 4} className={`w-full py-4 font-black uppercase tracking-widest rounded-xl transition-all shadow-lg ${inputCode.length === 4 ? 'bg-purple-600 hover:bg-purple-500 text-white hover:shadow-purple-500/25' : 'bg-slate-700 text-white/30 cursor-not-allowed'}`}>
...
// --- COMPONENTE ADMIN PANEL (FLOTANTE) ---
const AdminPanel = ({ players, onStartGame, onNextTurn, onReset, onKick }: any) => {
    return (
        <div className="bg-slate-800 p-4 rounded-xl border border-white/10 mb-6 w-full animate-in slide-in-from-top-4 relative z-50 shadow-2xl">
            <h3 className="text-xl font-bold text-pink-500 mb-4 border-b border-white/10 pb-2 flex items-center gap-2"><Settings size={18}/> Admin Control Panel</h3>
            <div className="space-y-4">
                <div className="bg-black/40 p-3 rounded-lg">
                    <p className="text-[10px] text-white/50 uppercase tracking-widest mb-2">Quick Actions</p>
                    <div className="grid grid-cols-2 gap-2">
                         <button onClick={onStartGame} className="bg-emerald-600 hover:bg-emerald-500 p-3 rounded-lg text-xs font-bold text-white transition-colors">Start Game / Setup</button>
                         <button onClick={onNextTurn} className="bg-blue-600 hover:bg-blue-500 p-3 rounded-lg text-xs font-bold text-white transition-colors">Force Next Turn</button>
                    </div>
                </div>
                <button onClick={onReset} className="w-full bg-red-900/50 hover:bg-red-900/80 border border-red-500/50 p-3 rounded-lg text-xs font-bold text-red-200 transition-colors flex items-center justify-center gap-2"><Trash2 size={14}/> Reset Entire Game</button>
                
                <div className="bg-black/40 p-3 rounded-lg max-h-40 overflow-y-auto">
                     <p className="text-[10px] text-white/50 uppercase tracking-widest mb-2">Player Management</p>
                     {players.map((p: any) => (
                         <div key={p.uid} className="flex justify-between items-center text-xs py-2 border-b border-white/5 last:border-0">
                             <span className="font-bold text-white">{p.name}</span>
                             <button onClick={() => onKick(p.uid, p.name)} className="text-red-400 hover:text-red-300 bg-red-900/20 px-2 py-1 rounded">Kick</button>
                         </div>
                     ))}
                </div>
            </div>
...
export default function TruthAndDareApp() {
    ...
    useEffect(() => {
      ...
      if (isNowAdmin) {
        ...
        setCode(autoCode);
        // Reset params when admin logs in
    ...
    // Levels Calculation
    useEffect(() => {
        const allLevels = new Set([...challenges.map(c => c.level), ...pairChallenges.map(c => c.level)]);
        const sorted = Array.from(allLevels).filter(l => l).sort((a, b) => parseInt(a) - parseInt(b));
        setUniqueLevels(sorted);
    }, [challenges, pairChallenges]);

    // Fetch Individual Card Details if needed
    useEffect(() => {
    ...
    // -----------------------------------------------------------------------
    // 3. LÓGICA GLOBAL SEGURA (Calculada aquí para evitar crash)
    // -----------------------------------------------------------------------
    let localCard = null;
    ...
    // -----------------------------------------------------------------------
    // 4. FUNCIONES DEL JUEGO Y LOGICA INTERNA
    // -----------------------------------------------------------------------
    ...
    const getCardText = (c: Challenge | undefined) => {
        if (!c) return 'Loading...'; 
        if (c.male || c.female) return `${c.male} / ${c.female}`;
    ...
    const findNextAvailableChallenge = async (type: string, startLevel: string, playerGender: string) => { 
        ...
        // Intentamos buscar en el nivel actual y los siguientes
        for(let i = 0; i < 10; i++) { 
    ...
    const nextTurn = async () => { 
        ...
        // 1. Calcular Puntos de la ronda actual
        ...
        if (gameState.mode === 'yn') { 
            ...
            // Saltar bots en turnos individuales
            while(nextIdx < players.length && players[nextIdx].isBot) { nextIdx++; } 
            
            if (nextIdx < players.length) { 
                // Sigue la misma ronda, siguiente jugador
                ...
                if (nextChallenge) { 
                    ...
                    roundFinished = true; // No hay cartas, fin de ronda
                } 
            } else { 
                roundFinished = true; // Fin de ronda
            } 
        } 
        
        // 3. Manejar Fin de Ronda (Manual o Auto)
        if (roundFinished) { 
    ...
    // ----------------------------------------------------------------------------------
    // 5. RENDERIZADO DEL ADMIN (HOST VIEW & SETUP)
    // ----------------------------------------------------------------------------------
    if (isAdmin && !viewAsPlayer) {
        // VISTA: JUEGO ACTIVO (Host View)
        ...
        // VISTA: SETUP ROUND (Configuración)
    ...
    // ----------------------------------------------------------------------------------
    // 6. VISTA DE JUGADOR (LOGIN + LOBBY + JUEGO)
    // ----------------------------------------------------------------------------------
    
    // A) PANTALLA DE INGRESO (LOGIN)
    if (!isJoined) {
        return (
     ...
     // C) INTERFAZ FINAL DEL JUEGO ACTIVO (RETURN FINAL)
     return (
           ...
           <div className="relative z-10 w-full max-w-md mx-auto flex flex-col items-center flex-1 p-4 animate-in fade-in">
                ...
                <div className="w-full flex justify-between items-center mb-4">
                    <div className="flex items-center gap-2">
                        {isAdmin && (
                            <button onClick={() => setShowAdminPanel(!showAdminPanel)} className={`p-2 rounded-full transition-all ${showAdminPanel ? 'bg-pink-500 text-white' : 'bg-white/10 text-slate-400'}`}>
                ...
                {isAdmin && showAdminPanel && (
                    <AdminPanel players={players} onStartGame={()=>{}} onNextTurn={nextTurn} onReset={handleRestart} onKick={handleKickPlayer} />
                )}
